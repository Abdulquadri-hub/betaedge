        ...
        <div class="max-w-3xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div v-if="currentStep < steps.length - 1" class="mb-8">
                <div class="flex justify-between mb-3">
                    <div v-for="(step, index) in steps.slice(0, -1)" :key="step.id"
            ...
            <Card>
                <CardContent class="p-6 md:p-8">
                    <AccountTypeStep v-if="currentStep === 0" :selected-type="enrollmentData.enrollmentType"
                        @select="handleAccountTypeSelect" />

                    <div v-else-if="currentStep === 1">
                        <template v-if="enrollmentData.enrollmentType === 'parent'">
                            <ParentRegistrationStep :data="enrollmentData.parent"
                                @update="(data) => Object.assign(enrollmentData.parent, data)" />
                            <ChildRegistrationStep :data="enrollmentData.child"
                                @update="(data) => Object.assign(enrollmentData.child, data)" />
                        </template>
                        <template v-else>
                            <AdultRegistrationStep :data="enrollmentData.adult"
                        ...
                        <div class="flex justify-between mt-6">
                            ...
                            <button @click="handleRegistrationComplete" :disabled="!canProceed"
                    ...
                    <CourseSelectionStep v-else-if="currentStep === 2"
                        ...
                        @update="handleCourseSelection" @next="nextStep" @back="prevStep" />

                    <BatchSelectionStep v-else-if="currentStep === 3" :selected-courses="enrollmentData.selectedCourses"
                        :preselected-batches="enrollmentData.selectedBatches"
                        @update="(data) => handleBatchSelection(data.selectedBatches)" @next="nextStep"
                        @back="prevStep" />

                    <PaymentStep v-else-if="currentStep === 4" :enrollment-data="enrollmentData"
                        @submit="handlePaymentSubmit" @back="prevStep" />

                    <!-- Step 5: Success -->
                    <EnrollmentSuccessStep v-else-if="currentStep === 5" :enrollment-data="enrollmentData"
